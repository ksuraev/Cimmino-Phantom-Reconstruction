 ...
 * @brief Implementation of the MTLRenderEngine class for rendering using Metal
 * and GLFW. This class handles the setup of a GLFW window with a CAMetalLayer,
...
void MTLRenderEngine::createRenderPipeline() {
  ...
  MTL::TextureDescriptor *texDesc = MTL::TextureDescriptor::texture2DDescriptor(
      MTL::PixelFormatRGBA32Float, numColors, 1, false);
  colourMapTexture = device->newTexture(texDesc);
  colourMapTexture->replaceRegion(MTL::Region(0, 0, numColors, 1), 0,
                                  colourMapData.data(),
  ...
  MTL::RenderPipelineDescriptor *renderDesc =
      MTL::RenderPipelineDescriptor::alloc()->init();
  renderDesc->setVertexFunction(vertexFn);
  renderDesc->setFragmentFunction(fragmentFn);
  renderDesc->colorAttachments()->object(0)->setPixelFormat(
      static_cast<MTL::PixelFormat>(metalLayer.pixelFormat));
  ...
  if (!renderPipeline) {
    std::cerr << "Error: Failed to create render pipeline state: "
              << error->localizedDescription()->utf8String() << std::endl;
    exit(-1);
  }
}
...
void MTLRenderEngine::render() {
  while (!glfwWindowShouldClose(glfwWindow)) {
    ...
    CA::MetalDrawable *drawable =
        getNextDrawable(reinterpret_cast<CAMetalLayer *>(metalLayer));
    if (!drawable) {
...
void MTLRenderEngine::initWindow() {
  ...
  glfwWindow = glfwCreateWindow(400, 400, "Cimmino's Phantom Reconstruction",
                                NULL, NULL);
  if (!glfwWindow) {
    glfwTerminate();
    exit(EXIT_FAILURE);
