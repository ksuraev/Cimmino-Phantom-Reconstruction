 ...
 * reconstruction. This class handles the generation of the projection matrix,
 ...
 * include/kernels.metal for Metal shader implementations.
 */
...
MTL::Function *MTLComputeEngine::createKernelFn(const char *functionName) {
  MTL::Function *fn = defaultLibrary->newFunction(
...
void MTLComputeEngine::dispatchThreads(MTL::ComputeCommandEncoder *encoder,
                                       MTL::ComputePipelineState *pipeline,
...
void MTLComputeEngine::generateProjectionMatrix(
  ...
  SparseMatrix matrix;
  loadSparseMatrixBinary(basePath + "/metal-data/" + projectionFileName, matrix,
                         header);
  ...
  if (matrix.rows.size() != (totalRays + 1)) {
    std::cerr << "Error: Offsets size does not match total rays + 1."
              << std::endl;
    exit(-1);
  }
  if (matrix.cols.size() != totalNonZeroElements ||
    ...
    std::cerr
        << "Error: Cols or Vals size does not match total non-zero elements."
        << std::endl;
    exit(-1);
  }
  ...
  offsetsBuffer =
      device->newBuffer(matrix.rows.data(), (matrix.rows.size()) * sizeof(int),
                        MTL::ResourceStorageModeShared);
  colsBuffer =
      device->newBuffer(matrix.cols.data(), matrix.cols.size() * sizeof(int),
                        MTL::ResourceStorageModeShared);
  valsBuffer =
      device->newBuffer(matrix.vals.data(), matrix.vals.size() * sizeof(float),
                        MTL::ResourceStorageModeShared);

  if (!offsetsBuffer || !colsBuffer || !valsBuffer) {
    std::cerr << "Error: Failed to create projection matrix buffers. "
              << std::endl;
    exit(-1);
  }
...
void MTLComputeEngine::performScan(std::vector<float> &phantomData) {
  ...
  std::vector<float> flippedPhantomData =
      flipImageVertically(phantomData, geom.imageWidth, geom.imageHeight);

  // Initialise texture for phantom
  auto phantomTextureDesc = MTL::TextureDescriptor::texture2DDescriptor(
      MTL::PixelFormatR32Float, geom.imageWidth, geom.imageHeight, false);
  phantomTextureDesc->setUsage(MTL::TextureUsageShaderRead);
  originalPhantomTexture = device->newTexture(phantomTextureDesc);
  phantomBuffer = device->newBuffer(flippedPhantomData.data(),
                                    flippedPhantomData.size() * sizeof(float),
                                    MTL::ResourceStorageModeShared);

  // Load original phantom data into texture
  MTL::Region region =
      MTL::Region::Make2D(0, 0, geom.imageWidth, geom.imageHeight);
  originalPhantomTexture->replaceRegion(region, 0, flippedPhantomData.data(),
                                        geom.imageWidth * sizeof(float));
  ...
  sinogramBuffer = device->newBuffer(totalRays * sizeof(float),
                                     MTL::ResourceStorageModeShared);
  if (!sinogramBuffer) {
  ...
  MTL::ComputePipelineState *scanPipeline =
      createComputePipeline(performScanFn);
  ...
  float stepSize = 2.0f;
  int numSteps =
      static_cast<int>(ceil(sqrt(float(geom.imageWidth * geom.imageWidth) +
                                 geom.imageHeight * geom.imageHeight) /
                            stepSize));
  ...
  auto sinoTextureDesc = MTL::TextureDescriptor::texture2DDescriptor(
      MTL::PixelFormatR32Float, geom.nDetectors, geom.nAngles, false);
  sinoTextureDesc->setUsage(MTL::TextureUsageShaderRead |
  ...
  blitEncoder->copyFromBuffer(sinogramBuffer, 0,
                              geom.nDetectors * sizeof(float), 0,
                              MTL::Size(geom.nDetectors, geom.nAngles, 1),
                              sinogramTexture, 0, 0, MTL::Origin(0, 0, 0));
...
void MTLComputeEngine::findMaxValInTexture(MTL::Texture *texture,
  ...
  MTL::ComputePipelineState *findMaxPipeline =
      createComputePipeline(maxPerThreadGroupFn);

  // Pass 2 pipeline - reduce to find final max
  auto reduceMaxFn = createKernelFn("reduceMaxKernel");
  MTL::ComputePipelineState *reduceMaxPipeline =
      createComputePipeline(reduceMaxFn);
  ...
  MTL::Size numThreadgroups =
      MTL::Size((gridSize.width + tgWidth - 1) / tgWidth,
                (gridSize.height + tgHeight - 1) / tgHeight, 1);
  ...
  MTL::Buffer *mapBuffer = device->newBuffer(numTgTotal * sizeof(float),
  ...
  NS::UInteger reduceTgSize =
      reduceMaxPipeline->maxTotalThreadsPerThreadgroup();
...
void MTLComputeEngine::normaliseTexture(MTL::Texture *texture,
  ...
  MTL::ComputePipelineState *normalisePipeline =
      createComputePipeline(normaliseFn);
  ...
  NS::UInteger normTgH =
      normalisePipeline->maxTotalThreadsPerThreadgroup() / normTgW;
...
MTLComputeEngine::reconstructImage(int numIterations, double &finalUpdateNorm) {
  ...
  // Create buffers
  reconstructedBuffer = device->newBuffer(imageSize * sizeof(float),
                                          MTL::ResourceStorageModeShared);
  auto updateBuffer = device->newBuffer(imageSize * sizeof(float),
                                        MTL::ResourceStorageModeShared);
  ...
  blit->fillBuffer(reconstructedBuffer, NS::Range(0, imageSize * sizeof(float)),
                   0.0f);
  blit->endEncoding();
  cmdBuffer->commit();
  cmdBuffer->waitUntilCompleted();

  std::cout << "Starting reconstruction for " << numIterations
            << " iterations..." << std::endl;
  ...
  std::chrono::duration<double, std::milli> totalReconstructTime =
      endTime - startTime;

  std::cout << "Reconstruction time for " << numIterations << " iterations "
            << totalReconstructTime.count() << " ms" << std::endl;
  ...
  MTL::TextureDescriptor *textureDesc =
      MTL::TextureDescriptor::texture2DDescriptor(MTL::PixelFormatR32Float,
                                                  width, height, false);
  ...
  if (!reconstructedTexture) {
    std::cerr << "Error: Failed to create reconstructed image texture."
              << std::endl;
    exit(-1);
  }

  // Copy reconstructed buffer into texture using blit encoder
  copyBlit->copyFromBuffer(reconstructedBuffer, 0, sourceBytesPerRow,
                           sourceBytesPerImage, MTL::Size(width, height, 1),
                           reconstructedTexture, 0, 0, MTL::Origin(0, 0, 0));
  copyBlit->endEncoding();
  cmdBuffer->commit();
  cmdBuffer->waitUntilCompleted();

  std::cout << "Reconstruction complete. Reconstructed image copied to texture."
            << std::endl;

  // Save reconstructed texture to file
  std::string imageFileName =
      basePath + "/metal-data/metal_" + std::to_string(numIterations) + ".txt";
