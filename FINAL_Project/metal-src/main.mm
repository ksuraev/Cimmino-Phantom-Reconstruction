...
int main(int argc, char **argv) {
  ...
  if (argc > 1) {
    numIterations =
        std::atoi(argv[1]); // Convert the first argument to an integer
  }
  ...
  try {
    ...
    auto numDetectors =
        static_cast<int>(std::ceil(2 * std::sqrt(2) * IMAGE_WIDTH));
    ...
    double projectionTime = timeMethod_ms([&]() {
      mtlComputeEngine.generateProjectionMatrix(PROJECTION_MATRIX_FILE);
    });

    // Generate sinogram for loaded phantom
    std::string basePath = PROJECT_BASE_PATH;
    std::vector<float> phantomData =
        loadPhantom(basePath + "/metal-data/" + PHANTOM_FILE, geom);
    double scanTime =
        timeMethod_ms([&]() { mtlComputeEngine.performScan(phantomData); });

    double finalErrorNorm = 0.0;
    // Perform cimmino reconstruction
    auto totalReconstructTime =
        mtlComputeEngine.reconstructImage(numIterations, finalErrorNorm);
    ...
    mtlRenderEngine.setReconstructedTexture(
        mtlComputeEngine.getReconstructedTexture());
    mtlRenderEngine.setOriginalPhantomTexture(
        mtlComputeEngine.getOriginalPhantomTexture());
    ...
    logPerformance(geom, numIterations, projectionTime, scanTime,
                   totalReconstructTime, finalErrorNorm,
                   basePath + "/metal-logs/metal_performance_log.csv");
